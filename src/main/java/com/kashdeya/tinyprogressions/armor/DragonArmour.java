...
public class DragonArmour extends BaseArmor {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        if (ArmorHandler.dragon_armor && ArmorHandler.dragon_fly) {
        	tooltip.add(new TranslationTextComponent("tooltip.dragonarmor_1").mergeStyle((TextFormatting.YELLOW)));
        }
        if (ArmorHandler.dragon_armor && ArmorHandler.dragon_strength) {
        	tooltip.add(new TranslationTextComponent("tooltip.dragonarmor_2").mergeStyle((TextFormatting.YELLOW)));
        }
        if (ArmorHandler.dragon_armor && ArmorHandler.dragon_fire) {
        	tooltip.add(new TranslationTextComponent("tooltip.dragonarmor_3").mergeStyle((TextFormatting.YELLOW)));
        }
        if (ArmorHandler.dragon_armor && ArmorHandler.dragon_resistance) {
        	tooltip.add(new TranslationTextComponent("tooltip.dragonarmor_4").mergeStyle((TextFormatting.YELLOW)));
        }
    }
}