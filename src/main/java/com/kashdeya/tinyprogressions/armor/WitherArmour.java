...
public class WitherArmour extends BaseArmor {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        if (ArmorHandler.wither_armor && (ArmorHandler.wither_strength || ArmorHandler.wither_fire || ArmorHandler.wither_resistance)) {
        	tooltip.add(new TranslationTextComponent("tooltip.witherarmor_1").mergeStyle((TextFormatting.YELLOW)));
        }
        if (ArmorHandler.wither_armor && ArmorHandler.wither_strength) {
        	tooltip.add(new TranslationTextComponent("tooltip.witherarmor_2").mergeStyle((TextFormatting.YELLOW)));
        }
        if (ArmorHandler.wither_armor && ArmorHandler.wither_fire) {
        	tooltip.add(new TranslationTextComponent("tooltip.witherarmor_3").mergeStyle((TextFormatting.YELLOW)));
        }
        if (ArmorHandler.wither_armor && ArmorHandler.wither_resistance) {
        	tooltip.add(new TranslationTextComponent("tooltip.witherarmor_4").mergeStyle((TextFormatting.YELLOW)));
        }
    }
}